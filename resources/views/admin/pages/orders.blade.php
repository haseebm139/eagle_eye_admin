...
<script>
    const customerOrders = [{
        ...
        },
        {
            ...
            trackingId: 'CUST67890',
            qty: 1,
            orderTotal: '$150',
            assignStatus: 'Shipped'
        },
        {
            id: 3,
    ...
    function renderCustomerOrdersTable() {
        ...
        customerOrdersToDisplay.forEach(order => {
            const statusClass = order.assignStatus === 'Delivered' ? 'text-success' : (order.assignStatus ===
                'Shipped' ? 'text-info' : 'text-warning');
            const row = `
        <tr>
          <td>
            <label class="custom-checkbox">
                    ...
                    if (labelElement) {
                        ...
                        if (assignButton) {
                            assignButton.innerHTML =
                                `${selectedName}   <img src="./assests/svg/fi_chevron-down2.svg" />`;
                            const dropdown = document.getElementById(`dropdown-${order.id}`);
                            if (dropdown) {
    ...
    function handleCmOrderSelectAll() {
        ...
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                } else if (document.querySelectorAll('.cm-order-checkbox:checked').length === checkboxes
                    .length) {
    ...
    function renderCustomerOrdersTable2() {
        ...
        customerOrdersToDisplay.forEach(order => {
            const statusClass = order.assignStatus === 'Delivered' ? 'custom-active' : (order.assignStatus ===
                'Shipped' ? 'custom-inactive' : 'custom-inactive');
            const row = `
      <tr>
        <td>
          <label class="custom-checkbox">
    ...
    `;
            ...
            document.getElementById(`assignList2-${order.id}`).addEventListener('click', function(e) {
                if (e.target.tagName === 'LABEL' || e.target.tagName === 'IMG') {
                    ...
                    assignButton2.innerHTML =
                        `${selectedName2} <img src="./assests/svg/fi_chevron-down2.svg" />`;
